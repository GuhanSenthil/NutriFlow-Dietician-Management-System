...
import { AppUser, Role, User, Patient, Payment, DieticianPayment, Dietician } from '../../types';
...
import DieticianPayrollTable from './DieticianPayrollTable';

const AdminDashboard: React.FC = () => {
    const [stats, setStats] = useState({ patients: 0, dieticians: 0, revenue: 0, monthlyPayout: 0 });
    const [patients, setPatients] = useState<Patient[]>([]);
    const [dieticians, setDieticians] = useState<AppUser[]>([]);
    const [payments, setPayments] = useState<Payment[]>([]);
    const [dieticianPayments, setDieticianPayments] = useState<DieticianPayment[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        const [patientsData, dieticiansData, paymentsData, dieticianPaymentsData] = await Promise.all([
            api.getUsersByRole(Role.PATIENT) as Promise<Patient[]>,
            api.getUsersByRole(Role.DIETICIAN),
            api.getAllPayments(),
            api.getDieticianPayments(),
        ]);
        
        const totalRevenue = paymentsData.reduce((sum, p) => p.status === 'completed' ? sum + p.amount : sum, 0);

        const currentMonthKey = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`;
        const totalPayout = dieticianPaymentsData
            .filter(p => p.forMonth === currentMonthKey)
            .reduce((sum, p) => sum + p.amount, 0);

        setPatients(patientsData);
        setDieticians(dieticiansData);
        setPayments(paymentsData);
        setDieticianPayments(dieticianPaymentsData);
        setStats({ 
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard title="Total Revenue" value={`₹${stats.revenue.toLocaleString()}`} icon={<Icon name="billing" className="h-6 w-6 text-primary-600"/>}/>
                <StatCard title="Monthly Payouts" value={`₹${stats.monthlyPayout.toLocaleString()}`} icon={<Icon name="billing" className="h-6 w-6 text-primary-600"/>}/>
            </div>
            ...
            <div className="space-y-6">
                <DieticianPayrollTable 
                    dieticians={dieticians as Dietician[]} 
                    payments={dieticianPayments}
                    onPaymentSuccess={fetchData}
                />
                <PaymentTransactionsTable payments={payments} patients={patients} isLoading={loading} />
