...
const PatientAcceptedView: React.FC<PatientAcceptedViewProps> = ({ patient, dietician }) => {
    ...
    const [selectedSlotForPayment, setSelectedSlotForPayment] = useState<string | null>(null);
    ...
    useEffect(() => {
        const checkPaymentStatus = async () => {
            ...
            try {
                const payments = await api.getPaymentsByPatientId(patient.id);
                const thirtyDaysAgo = new Date();
                thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

                const hasRecentPayment = payments.some(p => 
                    p.status === 'completed' && new Date(p.date) > thirtyDaysAgo
                );
                
                setHasActivePayment(hasRecentPayment);
            } catch (error) {
                console.error("Failed to check payment status", error);
                setHasActivePayment(false); // Default to no access on error
            } finally {
    ...
    const handlePaymentSuccess = async () => {
        ...
        await refreshUser();
        // Re-check payment status after successful payment
    ...
    const handleMessageClick = () => {
        if (hasActivePayment) {
            ...
            showToast('Messaging is enabled for 30 days after each appointment booking.', 'error', 5000);
        }
    };

    const tools = [
        { name: 'Log Food Diary', icon: 'book', action: () => setIsFoodDiaryModalOpen(true), disabled: false, loading: false },
        { name: 'Track Goals', icon: 'goal', action: () => setIsGoalTrackerModalOpen(true), disabled: false, loading: false },
        { name: 'Message Dietician', icon: 'send', action: handleMessageClick, disabled: !hasActivePayment && !isLoadingPaymentStatus, loading: isLoadingPaymentStatus },
        { name: 'Resource Hub', icon: 'clipboard', action: () => setIsResourceHubModalOpen(true), disabled: false, loading: false },
        { name: 'Book Appointment', icon: 'calendar', action: () => setIsAppointmentModalOpen(true), disabled: false, loading: false },
        { name: 'Billing', icon: 'billing', action: () => setIsBillingModalOpen(true), disabled: false, loading: false },
    ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <Card title="Tools & Resources">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {tools.map(tool => {
                            const isDisabled = tool.disabled || tool.loading;
                            return (
                                <div 
                                    key={tool.name} 
                                    onClick={!isDisabled ? tool.action : (tool.name === 'Message Dietician' ? handleMessageClick : () => {})} 
                                    className={`group relative flex flex-col items-center justify-center p-4 bg-gray-50 rounded-lg transition-all duration-300 ${isDisabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer border-2 border-transparent hover:border-primary-500 hover:shadow-lg hover:-translate-y-1'}`}
                                >
                                    {tool.loading && (
                                    ...
                                    <div className={`bg-primary-100 p-3 rounded-full ${!isDisabled ? 'group-hover:bg-primary-500' : ''} transition-colors`}>
                                        <Icon name={tool.icon} className={`h-6 w-6 text-primary-600 ${!isDisabled ? 'group-hover:text-white' : ''} transition-colors`} />
                                    </div>
            ...
            {dietician && (
                <ChatModal 
                    ...
                    patient={patient}
                    dietician={dietician}
                    isMessagingEnabled={hasActivePayment}
                />
            )}
            ...
            {dietician && selectedSlotForPayment && (
                <PaymentModal
                    ...
                    patient={patient}
                    dietician={dietician}
                    dateTime={selectedSlotForPayment}
                />
            )}
