...
const ChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose, patient, dietician, isMessagingEnabled }) => {
    const { user } = useAuth();
            ...
            <div className="flex flex-col h-[60vh]">
                ...
                <div className="p-4 border-t flex items-center bg-white rounded-b-md">
                    {isMessagingEnabled ? (
                        ...
                        <div className="w-full text-center p-2 bg-yellow-100 text-yellow-800 text-sm rounded-md border border-yellow-200">
                            <p className="font-semibold">Messaging Disabled</p>
                            <p>Please book an appointment to enable messaging for 30 days.</p>
                        </div>
                    )}
                </div>
            </div>
